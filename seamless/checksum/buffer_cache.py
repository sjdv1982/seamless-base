...
### from seamless.checksum import buffer_remote ##TODO
...
class BufferCache:
    ...
    _bottled_update_time_calls = []

    def __init__(self):
        self.buffer_cache: dict = {}  # local cache, checksum-to-buffer
        self.last_time: dict = {}
        self.buffer_refcount: dict = {}  # buffer-checksum-to-refcount
        # Buffer info cache (never expire)
        self.buffer_info: dict = {}  # checksum-to-buffer-info
        self.synced_buffer_info: set = (
            set()
        )  # set of bufferinfo checksums that are in-sync with the database
        self.missing: dict = {}  # key: checksum, value: persistent (bool)
        self.persistent_buffers: set = (
            set()
        )  # only used if LOCAL_MODE_FULL_PERSISTENCE is False

        self._bottled_update_time_calls: list = []

        self.incref_buffer(
            Checksum(empty_dict_checksum), Buffer(b"{}\n"), persistent=True
        )
        self.incref_buffer(
            Checksum(empty_list_checksum), Buffer(b"[]\n"), persistent=True
        )
    ...
    def find_missing(self, checksum: Checksum, buffer: Buffer | None, persistent=False):
    ...
    def guarantee_buffer_info(
        ...
        *,
        ...
        if celltype == "mixed":
            ...
            if buffer is not None:
                buf = buffer.content
                if buf.startswith(MAGIC_NUMPY):
                    self.update_buffer_info(
                        checksum, "is_numpy", True, sync_remote=False
                    )
                elif buf.startswith(MAGIC_SEAMLESS_MIXED):
                    self.update_buffer_info(
    ...
    def destroy(self):
        ...
        self.buffer_cache = None  # type: ignore
        self.last_time = None  # type: ignore
        self.buffer_refcount = None  # type: ignore
        self.buffer_info = None  # type: ignore
