...
def get_typedef_scalar(value):
    if isinstance(value, bool):
        typedef = "boolean"
    elif isinstance(value, _integer_types):  # type: ignore
        typedef = "integer"
    elif isinstance(value, _float_types):  # type: ignore
        typedef = "number"
    elif isinstance(value, _string_types):  # type: ignore
        typedef = "string"
    elif value is None:
        typedef = "null"
    else:
    ...
    if storage0 is None:  # typedef of a Python object inside Numpy
        ...
        if isinstance(typedef, dict) and "items" in typedef:
            ...
            if typedef["type"] in ("array", "tuple"):  # Numpy array of Python objects
                if (
                    typedef["type"] == "tuple"
                ):  # Numpy multi-element field of Python objects
                    assert data.shape == typedef["shape"]
                item_storage, item_typedef = [], []
                for (
                    d
                ) in data:  # TODO: multi-dimensional (currently NotImplementedError)
                    curr_item_storage, curr_item_typedef = get_form_python_inside_numpy(
                        d
                    )
                ...
                for dnr, d in enumerate(data[1:]):
                    if (
                        item_storage[dnr] != item_storage[0]
                        or item_typedef[dnr] != item_typedef[0]
                    ):
                        identical = False
                        break
                else:
...
def get_form_python_inside_numpy(data):
    if isinstance(data, list):
        ...
        raise TypeError(
            type(data)
        )  # must be list, dict, string or None; scalar Python objects not allowed


def get_tform_numpy_pyobject(dt):
    if dt.ndim:
        if dt.ndim > 1:
            raise NotImplementedError  # TODO: multi-dimensional Python tuples
...
def is_np_str(dt):
    return dt == np.dtype("S%d" % dt.itemsize) or dt == np.dtype(
        "U%d" % (dt.itemsize / 4)
    )
...
def get_form_dict_plain(data):
    ...
    props = {}
    typedef["properties"] = props  # type: ignore
    storages = {}
...
def get_form_list(data):
    ...
    typedef = {"type": "array", "items": items, "identical": identical}
